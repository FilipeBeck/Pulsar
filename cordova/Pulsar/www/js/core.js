...
{
	/**
	 * Definição base para todas as classes criadas com o framework. Embora seja documentada como uma classe abstrata com membros estáticos,
	 * ela é apenas um dicionário utilzado como molde para definir as respectivas propriedades estáticas das classes criadas por <b>Pulsar.class()</b>
	 * @class Pulsar~CoreDefinition
	 * @abstract
	 */
	function CoreDefinition(TheSuperclass, TheSuperclasses)
	{
		/** @var {CoreDefinition} Pulsar~CoreDefinition.Superclass */
		Object.defineProperty(this, 'Superclass', { value: TheSuperclass, writable: false });
		/** @var {CoreDefinition[]} Pulsar~CoreDefinition.Superclasses Todas as superclasses */
		Object.defineProperty(this, 'Superclasses', { value: TheSuperclasses, writable: false });
		/**
		...
		this.freezeDefinition = function()
		{
			Object.freeze(this.Superclass);
			Object.freeze(this.Superclasses);
	...
	/** @var {WeakMap} Pulsar~instances Todas as instãncias criadas pelo framework */
	var instances = new WeakMap();

	/** @var {WeakMap} Pulsar~privateDescriptors Descritores das propriedades e métodos privados de todas as classes */
	...
	{ // Primeiro propaga
		...
		var SuperclassScope = PrivateScopes.get(Superclass);
	...
	{ // Converte o argumento para um array verdadeiro
		...
		/** @class Pulsar~PrivateScope */
		var PrivateScope = function (){};
		CoreDefinition.call(PrivateScope, Superclasses[0], Superclasses);
		
		function PulsarConstructor()
		{
			var privateScope = {}
			instances.set(this, privateScope);
			...
			(function defineProperties(Class, instance)
			{
				...
				if (Class.descriptor != undefined)
				{
					...
					Object.defineProperties(privateScope, PrivateScopes.get(Class).descriptor['instance']['properties']);
				}
		...
		{
			PulsarConstructor.apply(this, arguments);
		})`);
		...
		for (var i in bodyDefinitions)
		{
			...
			if (accessLevel == 'public')
				...
				PrivateScope.descriptor[instanceOrStatic][propertyOrMethod][identifier] = definition;
		}
		...
		{
			NewClass.prototype = Object.create(NewClass.Superclass.prototype);
			
			var x = PrivateScopes.get(NewClass.Superclass)
			if (x != undefined)
			PrivateScope.prototype = Object.create(x.prototype);
		}
		else
		{ // TODO: Melhorar herança múltipla. Atualmente o nó está aglomerando tudo e tendo apenas 'Object.prototype' como bae
			var name = ''
			for (var i in Superclasses)
				name += Superclasses[i].name + '$';
			
			var multiClassPublic = eval(`(function ${name}(){})`);
			
			for (var i in Superclasses.reverse())
			{
				var Superclass = Superclasses[i];
				// Herda público e privado
				inheritPublicGenealogy(multiClassPublic, Superclass);
				inheritPrivateGenealogy(PrivateScope, Superclass);
			}
			
			NewClass.prototype = Object.create(multiClassPublic.prototype);
		}
		...
		Object.defineProperties(PrivateScope.prototype, PrivateScope.descriptor['instance']['methods']);
		PrivateScopes.set(NewClass, PrivateScope);
