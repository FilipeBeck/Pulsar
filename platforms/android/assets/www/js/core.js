...
/**
 * Utilizada na criação de classes, fornecendo um único método público, <b>Pulsar.class</b>
 * @namespace Pulsar
 */
Object.defineProperty(window, 'Pulsar', { value: new function Pulsar()
{
	...
	function CoreDefinition(Superclasses)
	{
		...
		this.construct = function(privateInstance, publicInstance)
		{
			for (var i in superConstructors)
				superConstructors[i](privateInstance, publicInstance);
	...
	{ // Converte o argumento para um array verdadeiro
		...
		var NewClass = $.currentNamespace[className] = eval(`(function ${className}()
		{
			...
			CoreConstructor(privateInstance, this);
		...
		for (var i in NewClass.bodyDefiner.definitions)
		{
			...
			if (checkList['cached'])
				descriptors['private'][instanceOrStatic][propertyOrMethod][identifier] = { value: null, writable: definition.writable };
