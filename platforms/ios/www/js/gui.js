...
{ 
	...
	commumStyle.innerHTML = '* { box-sizing: border-box; }'
	 ...
	 *
	 */
	$('static').animateBlock = function(duration, block, finish)
	{
		var sheet = document.styleSheets[0];
		
		if (sheet.insertRule)
			sheet.insertRule(`.pulsar-gui-transitioning { transition: ${duration}s; transform: translation3d(0,0,0); }`);
		else
			sheet.addRule('.pulsar-gui-transitioning', `transition: ${duration}s; transform: translation3d(0,0,0);`);
		
		var elements = document.getElementsByClassName('pulsar-gui-view');
		var nodeCount = elements.length;
		
		for (var i = 0, count = nodeCount; i < count; i++)
		{
			var node = elements[i];
			var originalClassName = node.className;
			node.className += ' pulsar-gui-transitioning';
			...
			node.addEventListener('transitionend', function(event) {
				...
				if (--nodeCount == 0)
				{
					sheet.removeRule('.pulsar-gui-transitioning');
