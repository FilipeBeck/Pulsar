...
Object.defineProperty($, 'load', { value: function(cssPath)
{ 
	var http = new XMLHttpRequest({mozAnon: false, mozSystem: true});
	...
	http.open('GET', cssPath, false); http.send();
	// Interpreta o módulo e registra o módulo como 'importado'
...
{
	/** @var {WeakMap} Pulsar~privateScopes Contém um mapa para cada classe definida, cada um com sua respectivas instâncias privadas */
	...
	function CoreDefinition(Superclass, Superclasses)
	{
		...
		this.privateScope = new WeakMap();
		// Cria o mapa para o escopo privado
		privateScopes.set(this, new WeakMap());
	...
	{ // Converte o argumento para um array verdadeiro
		...
		function PulsarConstructor()
		{
			...
			(function defineProperties(Class, instance)
			{
				...
				if (Class.descriptor != undefined)
				{
					...
					var privateDescriptor = PrivateDefinitions.get(Class).descriptor;
					Object.defineProperties(privateScope, privateDescriptor['instance']['properties']);
					//Object.defineProperties(privateScope, privateDescriptor['instance']['methods']);
					
					var methods = privateDescriptor['instance']['methods'];
					
					for (var method in methods)
						Object.defineProperty(privateScope, method, { value: PrivateDefinition.prototype[method].bind(instance) });
		...
		for (var i in bodyDefinitions)
		{
			...
			var Scope = (accessLevel == 'public') ? NewClass : PrivateDefinition;
			// Evita duplicatas
			for (var i in Scope.descriptor[instanceOrStatic][propertyOrMethod])
				if (Scope.descriptor[instanceOrStatic][propertyOrMethod][i] == identifier)
					throw new SyntaxError('Duplicated identifier \'' + identifier + '\' in \'' + className + '.' + identifier + '\' definition');
			...
			Scope.descriptor[instanceOrStatic][propertyOrMethod][identifier] = definition;
		}
		...
		// Estabelece os membros estáticos
		Object.defineProperties(NewClass, NewClass.descriptor['static']['properties']);
		Object.defineProperties(NewClass, NewClass.descriptor['static']['methods']);
